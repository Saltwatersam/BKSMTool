...
// 
// This file was originally based on WEMFile.cs from the WEMSharp project,
...
public class WEMFile : IDisposable
{
    private MemoryStream _wemFile;
    ...
    public WEMFile(MemoryStream MSfile, WEMForcePacketFormat forcePacketFormat)
    {
        this._wemFile = MSfile;
    ...
    #region Methods
    public byte[] GenerateOGG(byte[] codebooks, bool inlineCodebooks, bool fullSetup)
    {
        if (inlineCodebooks && codebooks == null)
        {
            throw new ArgumentException("Only one of Inline Codebooks or Codebooks Location can be set");
        }
        else if (!inlineCodebooks && codebooks == null)
        {
            throw new ArgumentException("Either Inline Codebooks or Codebooks Location must be set");
        }
        using (MemoryStream oggFile = new MemoryStream())
        {
            using (OggStream ogg = new OggStream(oggFile))
            {
                ...
                {
                    GenerateOGGHeader(ogg, codebooks, inlineCodebooks, fullSetup, ref modeBlockFlag, ref modeBits);
                }
    ...
    private void GenerateOGGHeader(OggStream ogg, byte[] codebooks, bool inlineCodebooks, bool fullSetup, ref bool[] modeBlockFlag, ref uint modeBits)
    {
        ...
        //Comment Packet
        {
            ...
            byte[] vendor = Encoding.UTF8.GetBytes("Converted from Audiokinetic Wwise .wem by BKSMTool");
        ...
        //Setup Packet
        {
            ...
            {
                CodebookLibrary codebook = new CodebookLibrary(codebooks);
